								...
								<div>
									...
									<ul class="nav nav-tabs" role="tablist" id="myTab">
										...
										<li role="presentation"><a href="#wishlisttab"
											...
											class="font-15 text-uppercase">위시리스트 <span class="badge">5</span></a></li>
									...
									<div class="tab-content">
																		...
																		<tr>
																			...
																			<td>
																				<button type="button" class="btn btn-success btn-xs"
																					data-toggle="modal"
																					data-target=".bs-example-modal-sm${dto.onOrderCode}">주문
																					내역 상세보기</button>
																					...
																					<div class="modal-dialog modal-sm">
																						<div class="modal-content">
																							<div class="modal-header">
																								...
																								<h4 class="modal-title" id="myModalLabel3">구매
																									상세 내역</h4>
																							</div>
																							<div class="modal-body">
																								<div class="card-content">
																									<div class="total_amount_container">
																										<div class="total_amount">${dto.onOrderCode}
																										</div>
																										<table class="table table-hover">
																											<tbody>
																												<tr>
																													<th>총 결제 금액</th>
																													<td><span style="text-align: right"><fmt:formatNumber
																																value="${dto.onOrderPrice}"
																																pattern="₩#,###" /></span></td>
																												</tr>
																												<c:forEach var="detail" items="${orderList}">
																													<c:if
																														test="${detail.onOrderCode eq dto.onOrderCode}">
																														<tr>
																															<td>
																																${detail.onlecture.onLectureName}</td>
																															<td><fmt:formatNumber
																																	value="${detail.onlecture.onLecturePrice}"
																																	pattern="₩#,###" /></td>
																														</tr>
																													</c:if>
																												</c:forEach>
																												...
																												<tr>
																													<th>주문 시각</th>
																													<td><time
																															datetime="Sat Dec 28 2019 14:27:27 GMT+0900 (GMT+09:00)">${dto.onOrderRegdate}</time></td>
																												</tr>
																												<tr>
																													<th>결제 수단</th>
																													<td>${dto.onOrderMethod}</td>
																							...
																							<div class="modal-footer">
																								...
																								<button type="button"
																									class="btn btn-primary text-white">문의하기</button>
																							</div>
																						</div>
																					</div>
																				</div>
										...
										<div role="tabpanel" class="tab-pane" id="bookmarks">
											<table class="table">
												<tbody>
													<tr>
														<th scope="row"><img src="http://placehold.it/100x50"
															alt=""></th>
        ...
        $('#myTab li:eq(2) a').on('click', function () {
            $.ajax({
                ...
                success : function(result){
    				...
    				$.each(result,function(index,item){
    					str+='<tr>';
    					str+='<td>'+"<a href=\"${pageContext.request.contextPath}/onLecture/detail/" + item.onLecture.onLectureNo + "\">" + item.onLecture.onLectureNo + "</a>";
    					str+='<td>'+"<a href=\"${pageContext.request.contextPath}/onLecture/detail/" + item.onLecture.onLectureNo + "\">" +item.onLecture.onLectureName+ "</a>";
    					str+='<td>'+item.onLecture.onLecturePrice+'</td>';
                        str += '<td><input type="button" value="삭제" id=' + item.wishListNo + '></td>';
                        str += '</tr>';
                    });
